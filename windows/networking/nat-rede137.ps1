#==============================================================================
# Script: Configurar-NAT-Rede137.ps1
# Descricao: Configura NAT no Windows 11 Pro para faixa 192.168.137.x
# Interface: Conexao Local* 10 - identificada pelo MAC (imutavel)
...
Write-Host "=========================================="
Write-Host "  Configurando NAT - Rede 192.168.137.x  "
Write-Host "=========================================="
Write-Host ""
...
try {
    $existingNat = Get-NetNat -Name "Rede137" -ErrorAction SilentlyContinue
    if ($existingNat) {
        $existingNat | Remove-NetNat -Confirm:$false
        Write-Host "   Regra NAT 'Rede137' antiga removida"
    }
    New-NetNat -Name "Rede137" -InternalIPInterfaceAddressPrefix "192.168.137.0/24" -ErrorAction Stop
    Write-Host "   Regra NAT 'Rede137' criada para 192.168.137.0/24"
} catch {
...
Get-NetIPAddress -InterfaceAlias $InterfaceAlias | Where-Object {$_.IPAddress -like "192.168.137.*"} | `
...
Write-Host "      - DNS: 192.168.137.1 ou 8.8.8.8"
Write-Host "      - Faixa: 192.168.137.10 a 192.168.137.200"
Write-Host ""
Write-Host "PARA TESTAR:"
Write-Host "   - Conecte um dispositivo na interface"
Write-Host "   - Verifique se recebeu IP 192.168.137.x"
